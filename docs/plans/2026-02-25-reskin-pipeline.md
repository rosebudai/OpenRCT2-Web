...
|-----------|---------|--------------|
| `batch_env.py` | `scripts/reskin/batch_sprites.py` | Remove NFO parsing. Input is just a PNG directory. No sprite ID ranges — discover files from disk. |
| `generate_restyled.py` | `scripts/reskin/generate_restyled.py` | Nearly identical. Update default paths. Keep FAL.ai logic as-is. |
| `parse_and_replace.py` | `scripts/reskin/parse_restyled.py` | Remove GRF rebuild / sheet patching. Output is restyled PNGs to a target dir. Keep alpha restore + edge blending. |
| `restyle_env.py` | `scripts/reskin/reskin.py` | Replace subprocess calls with direct function imports. Add `--workspace` mode. |
| `env_categories.py` | `scripts/reskin/categories.py` | Replace zBase sprite ID ranges with OpenRCT2 style presets (scenery, terrain, rides). No ID ranges needed. |

## Reference: existing OpenRCT2-Web reskin scripts

- `scripts/init-reskin-object-workspace.sh` — creates `<workspace>/sprites/` with exported PNGs + `sprites.json`
- `<workspace>/rebuild-object.sh` — rebuilds `images.dat` + `reskinned.parkobj` from sprites
- `scripts/create-reskin-overlay.sh` — maps workspaces into overlay dirs
- `scripts/build-reskin-upload.sh` — merges overlays into web upload zip
...
**Files:**
- Create: `scripts/reskin/categories.py`
...
Run: `python3 -c "import sys; sys.path.insert(0, 'scripts/reskin'); from categories import CATEGORIES; print(f'{len(CATEGORIES)} categories loaded')" `
...
**Files:**
- Create: `scripts/reskin/batch_sprites.py`
...
Create `scripts/reskin/test_batch_sprites.py`:
...
Run: `cd /workspace/OpenRCT2-Web && python3 -m pytest scripts/reskin/test_batch_sprites.py -v`
...
**Files:**
- Create: `scripts/reskin/generate_restyled.py`
...
Add to `scripts/reskin/test_generate_restyled.py`:
...
Run: `cd /workspace/OpenRCT2-Web && python3 -m pytest scripts/reskin/test_generate_restyled.py -v`
...
**Files:**
- Create: `scripts/reskin/parse_restyled.py`
...
Create `scripts/reskin/test_parse_restyled.py`:
...
Run: `cd /workspace/OpenRCT2-Web && python3 -m pytest scripts/reskin/test_parse_restyled.py -v`
...
**Files:**
- Create: `scripts/reskin/reskin.py`
...
Create `scripts/reskin/test_reskin.py`:
...
Run: `cd /workspace/OpenRCT2-Web && python3 -m pytest scripts/reskin/test_reskin.py -v`
Expected: All 6 tests PASS.
...
Run: `cd /workspace/OpenRCT2-Web && python3 -m pytest scripts/reskin/ -v`
...
```bash
python3 scripts/reskin/batch_sprites.py --help
python3 scripts/reskin/generate_restyled.py --help
python3 scripts/reskin/parse_restyled.py --help
python3 scripts/reskin/reskin.py --help
```

**Step 3: Verify category listing**

Run: `python3 scripts/reskin/reskin.py --list-categories`
